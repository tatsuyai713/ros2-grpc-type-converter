...
| ファイル | 役割 |
| --- | --- |
| `install_grpc.sh` | gRPC v1.66.1 をソースからビルドし `/opt/grpc` にインストール |
| `install_library.sh` | IDL→Proto 変換 → ライブラリコンパイル → ヘッダー生成を一括実行し `/opt/grpc-libs` にインストール |
| `build_examples.sh` | サンプルプロジェクトをビルド |
...
| `include/rclcpp/rclcpp.hpp` | **ROS 2 互換 API レイヤー** — Publisher / Subscription / Service / Client / Timer / Node |
| `include/grpc_rcl.hpp` | 後方互換ラッパー（非推奨、rclcpp.hpp の利用を推奨） |
...
3 つのスクリプトを順に実行するだけでセットアップが完了します。
...
ROS 2 メッセージ型の変換・コンパイル・ヘッダー生成をまとめて実行し、`/opt/grpc-libs` にインストールします。
...
1. IDL → Proto 変換 (`convert_idl_to_proto.py`)
2. 共有ライブラリのコンパイル・インストール (`compile_install_ros_msgs.sh`)
3. ROS 2 スタイルのラッパーヘッダー生成 (`make_access_header.py`)
...
### サーバー（Subscriber Node）の実行

```bash
LD_LIBRARY_PATH=/opt/grpc/lib:/opt/grpc-libs/lib ./subscriber_node
```

### クライアント（Publisher Node）の実行

別のターミナルで:

```bash
LD_LIBRARY_PATH=/opt/grpc/lib:/opt/grpc-libs/lib ./publisher_node
```

### サービスサーバーの実行

```bash
LD_LIBRARY_PATH=/opt/grpc/lib:/opt/grpc-libs/lib ./service_server
```
...
```bash
LD_LIBRARY_PATH=/opt/grpc/lib:/opt/grpc-libs/lib ./service_client
```
...
| `sensor_msgs` | Image, PointCloud2, LaserScan, Imu 等 |
...
ros2-grpc-type-converter/
├── README.md
├── install_grpc.sh              # Step 1: gRPC インストール
├── install_library.sh           # Step 2: ライブラリ一括インストール
├── build_examples.sh            # Step 3: サンプルビルド
├── convert_idl_to_proto.py      # IDL → Proto 変換 (install_library.sh から呼ばれる)
├── compile_install_ros_msgs.sh  # ライブラリコンパイル (install_library.sh から呼ばれる)
├── make_access_header.py        # ヘッダー生成 (install_library.sh から呼ばれる)
├── cmake/
│   └── common.cmake             # CMake 共通設定
├── include/                     # ROS 2 互換 API ヘッダー
│   ├── rclcpp/
│   │   └── rclcpp.hpp
│   └── grpc_rcl.hpp             # 後方互換ラッパー（非推奨）
├── grpc_example/                # サンプルプロジェクト
│   ├── CMakeLists.txt
│   └── src/
│       ├── client.cpp           # Publisher サンプル (ROS 2 スタイル)
│       ├── server.cpp           # Subscriber サンプル (ROS 2 スタイル)
│       ├── service_server.cpp   # Service Server サンプル
│       ├── service_client.cpp   # Service Client サンプル
│       └── test_all.cpp         # 自動テスト
...
**解決策**: `install_grpc.sh` で gRPC が正しくインストールされたか確認してください。
...
**解決策**: `install_library.sh` を再実行してください。
